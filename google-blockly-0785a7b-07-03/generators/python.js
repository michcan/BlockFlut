...
Blockly.Python.init = function(workspace) {
  ...
  +"s.start() \n"
  +"s.amp=0 \n"//évite click au démarrage
  +"time.sleep(0.2) \n"
  +"s.amp=1 \n"
  +"def signal_term_handler(signal, frame): \n"//évite click quand on stop
  +"  s.amp=0 \n"
  +"  time.sleep(0.1) \n"
  +"  s.shutdown() \n"
  +"  sys.exit(0) \n"
  ...
  Blockly.Python.HardCodeDef="silenceTable = LinTable([(0,0),(8191,0)]) \n"
  ...
  for (var i = 0; i < variables.length; i++) {
    defvars[i] = "self."+Blockly.Python.variableDB_.getName(variables[i],
...
Blockly.Python.finish = function(code) {
  ...
  var ClassInit=
  "class Steps(): \n"
  + this.prefixLines("def __init__(self): \n", this.INDENT);
  ...
  {
	SeqVar =
	"self.seq"+x+"=Seq(time=0.001,seq=["+this.seqs[x].join(',')+"]).play()\n"
	+"self.tf"+x+"=TrigFunc(self.seq"+x+",self.loop"+x+")\n"
	+"self.count"+x+"=0\n"
  ...
  return Blockly.Python.LaunchPyoServ 
  + Imports 
  + Blockly.Python.HardCodeDef 
